...
`obsctl strategy`가 비어 있거나 stderr에 warning을 출력하면, vault 루트의 `CLAUDE.md`를 대신 읽어 운영 지침으로 사용합니다:

```bash
obsctl read CLAUDE.md
```

`CLAUDE.md`도 없으면 사용자에게 다음을 안내합니다:
- vault에 전략 문서를 만들 것
- `~/.config/obsctl/config.json`의 `"strategy"` 필드에 파일명 등록
- 또는 vault 루트에 `CLAUDE.md`를 둬서 fallback으로 사용

## Step 2: 사용자 요청 분석

인자 `$ARGUMENTS`를 분석하여 작업을 결정합니다:
